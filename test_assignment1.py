...
# ----------------------------------------------------------------------
# Helper: run assignment1.py once with a scripted sequence of user inputs
# ----------------------------------------------------------------------
def run_script(inputs, monkeypatch):
    """
    Execute assignment1.py exactly once with the supplied `inputs`
    ...
    sys.modules.pop("assignment1", None)           # forget previous import
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        importlib.import_module("assignment1")     # import executes the menu loop once
